...
namespace MoreMountains.Tools
{
	...
	public static class MMDebug
	{
		...
		public static RaycastHit2D RayCast(Vector2 rayOriginPoint, Vector2 rayDirection, float rayDistance, LayerMask mask, Color color, bool drawGizmo = false)
		{
			...
			int HoleMask = 29; //Mukeadd
			int HoleMaskLayerMask = 1 << HoleMask; //Mukeadd

			return RaycastWithHoleSkip(rayOriginPoint, rayDirection, rayDistance, mask, HoleMaskLayerMask);

			//return Physics2D.Raycast(rayOriginPoint,rayDirection,rayDistance,mask);		
		}

            public static RaycastHit2D RaycastWithHoleSkip(
                Vector2 origin,
                Vector2 direction,
                float distance,
                ...
                LayerMask holeMask)
            {
                // 1. Cast against Hole layers
                RaycastHit2D holeHit = Physics2D.Raycast(origin, direction, distance, holeMask);

                // Check: Did we hit the Hole Mask?
                if (holeHit.collider != null)
                {
                    // --- We are entering a hole ---

                    // Calculate where the ray would naturally end
                    Vector2 rayEndPos = origin + (direction * distance);

                    // CHECK A: Is the ray long enough to actually exit the hole?
                    // If the endpoint is still inside the collider, there is no exit point along this ray.
                    if (holeHit.collider.OverlapPoint(rayEndPos))
                    {
                        return new RaycastHit2D(); // Ray ends inside the hole -> No collision
                    }

                    // CHECK B: Find the specific exit point
                    // We cast BACKWARDS from the end of the ray towards the start.
                    // We limit the length to ensure we don't overshoot the entry point.
                    float backwardDistance = distance - holeHit.distance;
                    RaycastHit2D exitHit = Physics2D.Raycast(rayEndPos, -direction, backwardDistance, holeMask);

                    // If we found the exit point (the back wall of the hole)
                    if (exitHit.collider != null)
                    {
                        Vector2 exitPoint = exitHit.point;

                        // Calculate how much ray length is left after leaving the hole
                        float distTraveledToExit = Vector2.Distance(origin, exitPoint);
                        float remainingDistance = distance - distTraveledToExit;

                        if (remainingDistance > 0)
                        {
                            // 4. Cast the Final Ray
                            // Offset slightly by epsilon (0.001f) to avoid hitting the hole's exit wall again.
                            // This time, we ONLY mask for the Target (ignoring other holes/walls).
                            Vector2 secondRayOrigin = exitPoint + (direction * 0.001f);

                            return Physics2D.Raycast(secondRayOrigin, direction, remainingDistance, targetMask);
                        }
                    }
                }

				// Hole never happened. Check for target only
				var hit = Physics2D.Raycast(origin, direction, distance, targetMask);
			// if (hit.collider != null) Debug.Log($"Standart hit {direction}");
				return hit;
            }
